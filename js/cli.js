...
function onMD5In(inputFile, cliElm) {
    addLog("<div class='cli-text'id='md5PendingProgressText'>Parsing. Please wait.</div><progress id='md5PendingProgressBar'></progress>", cliElm);
    console.log(inputFile);
    var progBar = $(cliElm).find("#md5PendingProgressBar");
    ...
    fileReader.onload = function (e) {
        ...
        if (currentChunk < chunks) {
            ...
            $(cliElm).find('#md5PendingProgressText').remove();
            $(cliElm).find("#md5PendingProgressBar").remove();
            addLog("<div class='cli-text'>" + inputFile.files[0].name + " -> " +
                "MD5 -> " + spark.end().toUpperCase() + "</div>", cliElm);
            $(cliElm).find("#md5Input").remove();
        }
    };
            ...
            case "md5": {
                ...
                $.getScript(
                    ...
                    function (data, textStatus, jqxhr) {
                        addLog(
                            "<form><input type='file' id='md5Input' onchange='onMD5In(document.getElementById(`md5Input`), this.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode);'></form>"
                            , cliElm);
                    }
                );
